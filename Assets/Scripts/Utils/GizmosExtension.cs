...
namespace Utils
{
    public static class GizmosExtensions
    {
        ...
        {
            DrawWireArc(center, Vector2.one * radius, 360, segments, rotation);
        }
        ...
        /// <param name="segments"></param>
        /// <param name="rotation"></param>
        public static void DrawWireEllipse(Vector3 center, Vector2 radius, int segments = 20, Quaternion rotation = default(Quaternion))
        {
        ...
        public static void DrawWireArc(Vector3 center, Vector2 radius, float angle, int segments = 20, Quaternion rotation = default(Quaternion))
        {
            ...
            Gizmos.matrix = Matrix4x4.TRS(center, rotation, Vector3.one);
            Vector3 from = Vector3.forward * radius.y;
            var step = Mathf.RoundToInt(angle / segments);
            for (int i = 0; i <= angle; i += step)
            {
                var to = new Vector3(radius.x * Mathf.Sin(i * Mathf.Deg2Rad), 0, radius.y * Mathf.Cos(i * Mathf.Deg2Rad));
                Gizmos.DrawLine(from, to);
                from = to;
            }
